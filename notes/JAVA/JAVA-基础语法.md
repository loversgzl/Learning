...
/*枚举：你要使用这个类就得 new 一个对象出来，而当你用ecplise创建一个枚举类型时，在使用时是不需要再new的，它本身就创建好了几个对象在其内部，这也就是枚举和类的最大区别。
...
问：Java 中的变量不一定要初始化？
答：错误，一定会初始化，全局变量，对象中的变量（需要new出一块空间）或者是静态变量，是系统自动做的，对应基本类型顺序，0，0，0，0L，0.0f，0，'u0000',false，所有引用类型都是null。局部变量（就是某个函数中的变量）要手动，否则直接报错。
